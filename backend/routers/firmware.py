...
from services.firmware import (
	...
	deploy_firmware as deploy_firmware_service,
	download_firmware as download_firmware_service,
	get_compatible_devices as get_compatible_devices_service,
	get_firmware_by_id as get_firmware_by_id_service,
	get_firmware_by_status as get_firmware_by_status_service,
	get_firmware_device_types as get_firmware_device_types_service,
	reject_firmware as reject_firmware_service,
	upload_firmware as upload_firmware_service,
)
...
def deploy_firmware(
	...
	try:
		result = deploy_firmware_service(
			db=db,
...
def cloud_to_many_device(
	...
	try:
		result = cloud_to_many_device_service(
			db=db,
...
def get_compatible_devices(
	...
	try:
		result = get_compatible_devices_service(db=db, firmware_id=firmware_id)
		return result
	except ValueError as e:
		if "not found" in str(e).lower():
			raise HTTPException(status_code=404, detail=str(e))
...
async def upload_firmware(
	...
	try:
		result = await upload_firmware_service(
			db=db,
...
def get_firmware_device_types(
	...
	user = get_authenticated_user(authorization, db)
	return get_firmware_device_types_service(db=db, user=user)
...
def get_firmware_by_status(
	...
	try:
		results = get_firmware_by_status_service(db=db, status=status, user=user)
		return results
	except ValueError as e:
		raise HTTPException(status_code=400, detail=str(e))
...
def get_firmware_by_id(
	...
	user = get_authenticated_user(authorization, db)
	try:
		result = get_firmware_by_id_service(db=db, firmware_id=firmware_id, user=user)
		return result
	except ValueError as e:
		if "not found" in str(e).lower():
			raise HTTPException(status_code=404, detail=str(e))
...
def download_firmware(
	...
	user = get_authenticated_user(authorization, db)
	try:
		content = download_firmware_service(db=db, firmware_id=firmware_id, user=user)
		return Response(
			content=content,
...
def reject_firmware(
	...
	try:
		result = reject_firmware_service(
			db=db,
			firmware_id=firmware_id,
			manager_id=user.id,
...
def approve_firmware(
	...
	try:
		result = approve_firmware_service(
			db=db,
			firmware_id=firmware_id,
			manager_id=user.id,
		)
